...
class BotManager:
    ...
    async def start_first_cycle(self, config):
        ...
        try:
            ...
            if not await utils.check_min_notional(
                ...
                raise ValueError(
                    f"Order amount {safe_amount} * price {safe_price} is below minimum notional for {config.symbol}"
                )
    ...
    async def shift_grid(self, cycle: DcaCycle, config, current_price: float):
        ...
        try:
            ...
            await self.session.commit()
            logger.info(
                f"Сетка сдвинута. Новый первый ордер: {binance_res['id']} по цене {first_order.price:.2f}"
            )
