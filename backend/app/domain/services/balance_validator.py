...
class BalanceValidator:
    ...
    async def validate_for_sell(
        ...
        warning = self._check_deviation_warning(deviation_pct, available, expected_amount)
    ...
    def _determine_sell_amount(
        ...
        is_exact_match = (
            deviation_pct < BalanceThresholds.EXACT_MATCH_THRESHOLD_PCT
        )
        
        if is_exact_match:
            logger.info(
                f"[BalanceValidator] Используем точный баланс: {available:.8f}"
            )
            return available
