...
class DustManager:
    ...
    async def process_dust(
        self, amount: float, accumulated_dust: float, symbol: str
    ) -> DustResult:
        """
        ...
        return DustResult(sellable_amount=sellable_amount, new_dust=new_dust)

    def _truncate_to_precision(
        self, amount: float, precision: int | float, symbol: str = None
    ) -> float:
        ...
        return float(self.exchange.amount_to_precision(symbol or "BTC/USDT", amount))
