...
class OrderHandler:
    ...
    async def handle_filled_order(self, binance_order: dict):
        ...
        if not db_order:
            ...
            if cycle_with_tp:
                logger.warning(
                    f"[OrderHandler] TP-ордер {binance_id} найден через current_tp_order_id, но отсутствует в таблице orders. Создаем запись."
                )
                db_order = Order(
                ...
                logger.error(
                    f"[OrderHandler] Ордер {binance_id} не найден в БД и не является текущим TP-ордером. Пропускаем."
                )
                all_orders = await self.session.execute(select(Order))
                logger.info(
                    f"[OrderHandler] Всего ордеров в БД: {len(all_orders.scalars().all())}"
                )
                return

        if db_order.status == OrderStatus.FILLED:
            logger.info(
                f"[OrderHandler] Ордер {binance_id} уже обработан другой задачей (status=filled)"
            )
            return
    ...
    async def _handle_buy_fill(self, db_order, cycle, config, binance_order):
        ...
        if fee_info:
            if isinstance(fee_info, dict):
                ...
                if fee_currency == base_currency:
                    ...
                    logger.warning(
                        f"Неизвестная валюта комиссии: {fee_currency}, используем fallback"
                    )
            ...
            try:
                ...
                logger.warning(
                    f"Не удалось получить комиссию с биржи, используем fallback 0.1%: {e}"
                )
                fee_rate = Decimal("0.001")
                fee_qty = filled_qty * fee_rate
        ...
        order_cost = Decimal(
            str(binance_order.get("cost", db_order.price * float(filled_qty)))
        )
        ...
        if cycle.current_tp_order_id:
            try:
                ...
                stmt = (
                    update(Order)
                    .where(Order.binance_order_id == cycle.current_tp_order_id)
                    .values(status=OrderStatus.CANCELED)
                )
                await self.session.execute(stmt)
                logger.info(
                    f"[OrderHandler] Старый TP-ордер {cycle.current_tp_order_id} отменен и помечен как CANCELED в БД"
                )
            except Exception as e:
        ...
        try:
            ...
            if expected_amount > 0:
                ...
                logger.info(
                    f"[OrderHandler] Отклонение баланса: {deviation:+.8f} {base_asset} ({deviation_pct:.2f}%)"
                )
            ...
            if expected_amount > 0:
                if (
                    abs(available_base - expected_amount) / expected_amount < 0.001
                ):  # <0.1%
                    amount_to_sell = available_base
                    logger.info(
                        f"[OrderHandler] Используем точный баланс: {amount_to_sell:.8f} {base_asset}"
                    )
            ...
            if amount_to_sell <= 0:
                logger.error(
                    f"[OrderHandler] Финальное количество равно нулю или отрицательное: {amount_to_sell:.8f}"
                )
                return
            ...
            if isinstance(amount_precision, int):
                ...
                final_amount = float(
                    self.exchange.amount_to_precision(config.symbol, total_with_dust)
                )
            ...
            if final_amount <= 0:
                logger.error(
                    f"[OrderHandler] Финальное количество после округления равно нулю: {final_amount:.8f}"
                )
                return
            ...
            safe_price_for_calc = await self.utils.round_price(
            ...
            tp_price_adaptive = avg_price * (
                Decimal("1") + Decimal(str(effective_tp_pct)) / Decimal("100")
            )
            safe_price = await self.utils.round_price(
                config.symbol, float(tp_price_adaptive)
            )

            notional_value = final_amount * safe_price
            if not await self.utils.check_min_notional(
                config.symbol, final_amount, safe_price
            ):
                logger.warning(
            ...
            logger.error(
                f"[OrderHandler] Неожиданная ошибка при создании TP: {e}", exc_info=True
            )
            return
        ...
        if next_order:
            next_safe_amount = await self.utils.round_amount(
                config.symbol, next_order.amount
            )
            next_safe_price = await self.utils.round_price(
                config.symbol, next_order.price
            )

            if not await self.utils.check_min_notional(
                config.symbol, next_safe_amount, next_safe_price
            ):
                logger.warning(
                    f"[OrderHandler] Следующий ордер слишком мал (количество={next_safe_amount}, цена={next_safe_price}), пропускаем"
                )
            else:
                ...
                logger.info(
                    f"[OrderHandler] Следующий ордер создан: binance_id={next_order.binance_order_id}, order_id={next_order.id}"
                )
    ...
    async def _handle_tp_fill(self, db_order, cycle, config, binance_order: dict):
        ...
        if base_cost == 0:
            ...
            logger.warning(
                f"Стоимость не предоставлена биржей, рассчитано: {base_cost}"
            )
        ...
        if fee_info and isinstance(fee_info, dict):
            ...
            if fee_currency == "USDT" or fee_currency == "USD":
                ...
                logger.warning(
                    f"Валюта комиссии {fee_currency} не USDT, используем 0.1%"
                )
                fee_cost = base_cost * Decimal("0.001")
        else:
            fee_cost = base_cost * Decimal("0.001")
            logger.warning(
                f"Нет информации о комиссии с биржи, рассчитано 0.1%: {fee_cost}"
            )
        ...
        if old_ws_manager:
            logger.info(
                f"Остановка старого WebSocket менеджера для конфигурации {config.id}"
            )
