...
def upgrade() -> None:
    ...
    sa.ForeignKeyConstraint(['user_id'], ['users.id'], name=op.f('bot_configs_user_id_foreign')),
    sa.PrimaryKeyConstraint('id', name=op.f('bot_configs_pkey'))
    )
    op.create_table('dca_cycles',
    sa.Column('id', sa.Uuid(), nullable=False),
    ...
    sa.ForeignKeyConstraint(['config_id'], ['bot_configs.id'], name=op.f('dca_cycles_config_id_foreign')),
    ...
    sa.Column('status', sa.Enum('PENDING', 'FILLED', 'CANCELED', 'PARTIAL', name='orderstatus'), nullable=False),
    sa.ForeignKeyConstraint(['cycle_id'], ['dca_cycles.id'], name=op.f('orders_cycle_id_foreign')),
    sa.PrimaryKeyConstraint('id', name=op.f('orders_pkey')),
    sa.UniqueConstraint('binance_order_id', name=op.f('orders_binance_order_id_unique'))
    )
