...
def upgrade() -> None:
    ...
    for row in rows:
        ...
        encrypted_key = encrypt_api_key(api_key) if not _is_encrypted(api_key) else api_key
        encrypted_secret = encrypt_api_key(api_secret) if not _is_encrypted(api_secret) else api_secret
...
def downgrade() -> None:
    ...
    for row in rows:
        ...
        decrypted_secret = decrypt_api_key(api_secret) if _is_encrypted(api_secret) else api_secret
