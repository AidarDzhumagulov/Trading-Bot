<template>
  <div class="min-h-screen bg-slate-950 p-4 md:p-6 lg:p-8">
      ...
      <div class="flex items-center justify-between">
        <div>
          ...
          <p class="text-slate-400 mt-1">
            Облачный терминал для торговли по стратегии DCA
          </p>
        </div>
        <div class="flex items-center gap-4">
          ...
          <button
            @click="handleLogout"
            class="px-4 py-2 bg-slate-800 hover:bg-slate-700 border border-slate-700 rounded-lg text-slate-300 hover:text-slate-100 transition flex items-center gap-2 text-sm font-medium"
          >
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
      <div class="space-y-6">
        <div
          class="bg-slate-900 border border-slate-800 rounded-lg p-6 shadow-xl"
        >
            ...
            <div>
              ...
              <input
                ...
                :class="{
                  'border-red-500':
                    balanceError ||
                    (config.budget > balance.freeUsdt && balance.freeUsdt > 0),
                }"
              />
          ...
          <div class="space-y-4">
            <div>
              <label class="block text-sm font-medium text-slate-300 mb-2">
                Длина сетки (%)
                <span class="text-slate-500 text-xs ml-1"
                  >(Диапазон цен для размещения ордеров)</span
                >
              </label>
              <input
            ...
            <div>
              <label class="block text-sm font-medium text-slate-300 mb-2">
                ...
                <span class="text-slate-500 text-xs ml-1"
                  >(Расстояние от текущей цены)</span
                >
              </label>
              <input
            ...
            <div>
              <label class="block text-sm font-medium text-slate-300 mb-2">
                ...
                <span class="text-slate-500 text-xs ml-1"
                  >(Ордера на покупку для усреднения)</span
                >
              </label>
              <input
            ...
            <div>
              <label class="block text-sm font-medium text-slate-300 mb-2">
                ...
                <span class="text-slate-500 text-xs ml-1"
                  >(Процент увеличения объема каждого следующего ордера)</span
                >
              </label>
              <input
              ...
              <p class="text-slate-500 text-xs mt-1">
                0% = тот же объем, 50% = увеличение в 1.5 раза, 100% =
                увеличение в 2 раза
              </p>
            </div>
        ...
        >
          ...
          <div class="space-y-4">
            <div class="flex items-center gap-3">
              ...
              <label
                for="trailingEnabled"
                class="text-sm font-medium text-slate-300 cursor-pointer"
              >
                Включить Trailing Take Profit
              </label>
            </div>
            <p class="text-xs text-slate-500">
              Автоматически отслеживает максимальную цену и продает при откате,
              максимизируя прибыль
            </p>

            <div
              v-if="config.trailingEnabled"
              class="space-y-4 pt-2 border-t border-slate-700"
            >
              <div>
                <label class="block text-sm font-medium text-slate-300 mb-2">
                  Процент отката от максимума (%)
                  <span class="text-slate-500 text-xs ml-1"
                    >(Callback для продажи)</span
                  >
                </label>
                <input
                ...
                />
                <p class="text-slate-500 text-xs mt-1">
                  Рекомендуется: 0.5-1.2%. Автоматически адаптируется к
                  волатильности
                </p>
              </div>
              ...
              <div>
                ...
                />
                <p class="text-slate-500 text-xs mt-1">
                  Если цена падает ниже этого уровня - аварийная продажа по
                  рынку
                </p>
              </div>
            </div>
        ...
        <div
          v-if="errorMessage"
          class="mt-4 p-4 bg-red-900/30 border border-red-800 rounded-lg"
        >
        ...
        <div
          v-if="botStatus === 'active' && configId"
        ...
        >
          <div class="flex items-start gap-3">
            ...
            />
            <div class="flex-1">
              <p class="text-sm font-semibold text-emerald-400 mb-1">
                Бот активен
              </p>
              <p class="text-xs text-emerald-300">
                ID конфигурации: {{ configId }}
              </p>
              <p v-if="cycleId" class="text-xs text-emerald-300">
                ID цикла: {{ cycleId }}
              </p>
            </div>
          </div>
      ...
      <div class="space-y-6">
        <div class="grid grid-cols-2 gap-4">
          ...
          >
            ...
            <p
              v-if="dashboard.roiPct !== 0"
              class="text-xs mt-1"
              :class="
                dashboard.roiPct >= 0 ? 'text-emerald-400' : 'text-red-400'
              "
            >
              ROI: {{ dashboard.roiPct >= 0 ? "+" : ""
              }}{{ dashboard.roiPct.toFixed(2) }}%
            </p>
          </div>
        ...
        <div
          v-if="dashboard.completedCycles > 0"
        ...
        >
          <div class="flex items-center justify-between mb-4">
            ...
            <span
              ...
              :class="
                botStatus === 'active'
                  ? 'bg-emerald-900/50 text-emerald-500 border border-emerald-700'
          ...
          <div class="mb-6">
            <div class="flex justify-between text-sm mb-2">
              ...
              <span class="text-slate-300 font-semibold">
                {{ dashboard.currentCycle.filledOrders }} /
                {{ config.safetyOrdersCount }}
              </span>
            </div>
          ...
          <div class="grid grid-cols-2 gap-4">
            ...
            <div class="bg-slate-800/50 rounded-lg p-4">
              ...
              <p
                v-if="dashboard.currentCycle.effectiveTpPct > 0"
                class="text-xs text-emerald-400 mt-1"
              >
                Эффективный TP:
                {{ dashboard.currentCycle.effectiveTpPct.toFixed(2) }}%
              </p>
            </div>
            <div class="bg-slate-800/50 rounded-lg p-4">
          ...
          <div
            v-if="
              dashboard.currentCycle.unrealizedProfit !== 0 ||
              dashboard.currentCycle.expectedProfit !== 0
            "
          ...
          >
            <div class="bg-slate-800/50 rounded-lg p-4">
              ...
              >
                {{ dashboard.currentCycle.unrealizedProfit >= 0 ? "+" : "" }}${{
                  formatNumber(dashboard.currentCycle.unrealizedProfit)
                }}
              </p>
            </div>
            <div class="bg-slate-800/50 rounded-lg p-4">
          ...
          <div
            v-if="dashboard.currentCycle.accumulatedDust > 0"
            class="mt-4 bg-slate-800/30 rounded-lg p-3"
          >
            <p class="text-xs text-slate-400">
              Накопленная пыль:
              {{ dashboard.currentCycle.accumulatedDust.toFixed(8) }}
              {{ config.market.split("/")[0] }}
            </p>
          </div>
        </div>
        ...
        <div
          v-if="dashboard.trailingStats.trailingEnabled"
          class="bg-slate-900 border border-slate-800 rounded-lg p-6 shadow-xl"
        >
          <div class="flex items-center gap-2 mb-4">
            <TrendingUpIcon class="w-5 h-5 text-slate-400" />
            ...
            <div class="grid grid-cols-2 gap-4">
              ...
              <div class="bg-slate-800/50 rounded-lg p-4">
                ...
                <p class="text-lg font-semibold text-emerald-500">
                  {{
                    (
                      dashboard.trailingStats.statistics.successRatePct || 0
                    ).toFixed(1)
                  }}%
                </p>
              </div>
          ...
          <div
            v-if="dashboard.trailingStats.currentCycle"
            ...
            <div class="grid grid-cols-2 gap-4">
              <div class="bg-slate-800/50 rounded-lg p-4">
                ...
                <p class="text-lg font-semibold text-slate-100">
                  ${{
                    formatNumber(
                      dashboard.trailingStats.currentCycle.activationPrice || 0,
                    )
                  }}
                </p>
              </div>
              <div class="bg-slate-800/50 rounded-lg p-4">
                ...
                <p class="text-lg font-semibold text-emerald-500">
                  ${{
                    formatNumber(
                      dashboard.trailingStats.currentCycle.maxPriceTracked || 0,
                    )
                  }}
                </p>
              </div>
              <div class="bg-slate-800/50 rounded-lg p-4">
                ...
                <p class="text-lg font-semibold text-slate-100">
                  ${{
                    formatNumber(
                      dashboard.trailingStats.currentCycle.currentTpPrice || 0,
                    )
                  }}
                </p>
              </div>
              <div class="bg-slate-800/50 rounded-lg p-4">
                ...
                <p class="text-lg font-semibold text-emerald-500">
                  {{
                    dashboard.trailingStats.currentCycle.potentialProfitPct
                      ? dashboard.trailingStats.currentCycle.potentialProfitPct.toFixed(
                          2,
                        )
                      : "0.00"
                  }}%
        ...
        >
          ...
          <div class="flex items-end justify-between">
            <div>
              ...
              <p
                class="text-4xl font-bold text-slate-100 flex items-center gap-2"
              >
            ...
            <div class="text-right">
              ...
              >
                {{ dashboard.priceChange24h >= 0 ? "+" : ""
                }}{{ dashboard.priceChange24h.toFixed(2) }}%
              </p>
            </div>
          </div>
        </div>
          ...
          <div class="space-y-3 text-sm">
            <div class="flex justify-between">
              ...
              <span class="text-slate-100 font-semibold"
                >${{ formatNumber(calculateBaseOrderSize()) }}</span
              >
            </div>
            <div class="flex justify-between">
              <span class="text-slate-400">Диапазон сетки:</span>
              <span class="text-slate-100 font-semibold">
                ${{ formatNumber(calculateGridRange().min) }} - ${{
                  formatNumber(calculateGridRange().max)
                }}
              </span>
            </div>
            <div class="flex justify-between">
              <span class="text-slate-400"
                >Ожидаемая максимальная просадка:</span
              >
              <span class="text-red-500 font-semibold"
                >-{{ config.gridLength.toFixed(2) }}%</span
              >
            </div>
            <div class="flex justify-between">
...
const safetyOrderProgress = computed(() => {
  const progress =
    (dashboard.value.currentCycle.filledOrders /
      config.value.safetyOrdersCount) *
    100;
...
const validateBalance = async () => {
  ...
  try {
    ...
    if (balanceData.freeUsdt < MIN_BUDGET) {
      balanceError.value = `Недостаточно средств. Минимум ${MIN_BUDGET} USDT требуется. Доступно: ${balanceData.freeUsdt.toFixed(
        2,
      )} USDT`;
    } else if (config.value.budget > balanceData.freeUsdt) {
      balanceError.value = `Бюджет (${config.value.budget.toFixed(
        2,
      )} USDT) превышает доступный баланс (${balanceData.freeUsdt.toFixed(
        2,
      )} USDT)`;
    }
  } catch (error) {
...
const updateStats = async () => {
  ...
  try {
    ...
    dashboard.value.currentUnrealizedProfit =
      stats.currentUnrealizedProfit || 0;
    ...
    if (stats.currentCycle) {
      ...
      dashboard.value.currentCycle.unrealizedProfit =
        cycle.unrealizedProfit || 0;
...
const startBot = async () => {
  ...
  try {
    ...
    console.log(
      "Bot successfully started! Config ID:",
      configId.value,
      "Cycle ID:",
      cycleId.value,
    );
