...
apiClient.interceptors.response.use(
  ...
  async (error) => {
    ...
    if (error.response?.status === 401 && !originalRequest._retry) {
      ...
      try {
        const { getRefreshToken, saveTokens, clearTokens } = await import(
          "../stores/authStore.js"
        );
...
export const startBot = async (configId) => {
  try {
    const response = await apiClient.post(
      `/api/v1/bot_config/${configId}/start/`,
    );
    return convertKeysToCamelCase(response.data);
  } catch (error) {
    handleApiError(error);
...
export const stopBot = async (configId) => {
  try {
    const response = await apiClient.post(
      `/api/v1/bot_config/${configId}/stop/`,
    );
    return convertKeysToCamelCase(response.data);
  } catch (error) {
    handleApiError(error);
...
export const getTrailingStats = async (configId) => {
  try {
    const response = await apiClient.get(
      `/api/v1/bot_config/${configId}/trailing-stats/`,
    );
    return convertKeysToCamelCase(response.data);
  } catch (error) {
    handleApiError(error);
...
const handleApiError = (error) => {
  if (error.response) {
    ...
    throw new Error(
      "Network Error: Unable to reach the server. Check if backend is running.",
    );
  } else {
